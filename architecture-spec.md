...
| **DB_EN 双缓冲** | **INT8/INT16 DB_EN tiled, DB_EN+Fusion, DB_EN perf 基准** | **3 tests PASS** |
...
### 14.6 DB_EN 双缓冲 (Ping-Pong) 验证

DB_EN（Double-Buffer Enable）通过 ping-pong 调度隐藏 DMA 加载下一个 tile 输入数据的延迟，使计算与数据搬运并行执行。ACT SRAM 分为 Bank[0] 和 Bank[1]（各 ACT_DEPTH/2 字），计算引擎和 DMA 交替访问不同 bank。

**核心机制**：
- `ping_pong_flag` 控制计算引擎读取的 bank：0 读 Bank[0]，1 读 Bank[1]
- 计算当前 tile 的同时，DMA 预取下一 tile 的数据到另一 bank
- 每个 tile 的数据加载到 `bank_base + tile_word_offset`，其中 `tile_word_offset` 是该 tile 在完整张量中的全局字偏移
- `cfg_tile_in_size`（CSR 0x134）指定每 tile 输入字节大小

**实现要点**：
- TILE_IN_SIZE 寄存器（0x134）：当 `db_en=1 && tile_in_size>0` 时，控制器使用 `tile_in_size/4` 而非全 `in_size/4` 作为传输长度
- `db_prefetch_done` 握手信号：计算完成一个 tile 后进入 `S_TILE_WAIT_DB` 状态，等待预取 DMA 完成和 `ping_pong_flag` 翻转后再启动下一 tile
- 预取 DMA 的 SRAM 目标地址 = `bank_base + tile_word_offset`，确保计算引擎使用全局坐标读取时地址匹配

**验证结果**：

| 测试 | 描述 | 结果 | 仿真时间 |
|------|------|------|----------|
| test_dma_e2e_tiling_int8_db_en | 32×32 6层 INT8, 2×1 tiles, DB_EN | PASS (6/6) | 40.8M ns |
| test_dma_e2e_tiling_int16_db_en | 32×32 6层 INT16, 2×1 tiles, DB_EN | PASS (6/6) | 24.5M ns |
| test_dma_e2e_tiling_int8_db_en_fused | Conv1×1→DW→Conv1×1 融合块, DB_EN | PASS | 21.0M ns |
| test_dma_e2e_tiling_perf_db_en | DB_EN 性能基准 (MAC 计数 + 周期对比) | PASS | 9.5M ns |

验证环境：Icarus Verilog 11.0, SPAD_KB=128, ARRAY_SIZE=16。

### 14.7 Auto-Next 多层自动推理
...
RTL 已有 IRQ 基础设施（irq_en/irq_status/irq_o），无需额外 RTL 修改即可完成中断驱动推理。

| 测试 | 描述 | 结果 |
|------|------|------|
| test_irq_done | DONE 中断触发与 W1C 清除 | PASS |
| test_irq_mask | IRQ mask 屏蔽功能 | PASS |
| test_irq_multiple | 多中断源同时触发 | PASS |
| test_irq_e2e | 中断驱动推理完成（Conv2D E2E + W1C） | PASS |

关键发现：Conv2D 完成时 `hw_done` 和 `hw_dma_done` 同时触发，W1C 需清 `0x07`。

### 14.9 测试加固与全回归

| 测试 | 描述 | 结果 |
|------|------|------|
| INT8 宽参数模型 | weights [-64,65], bias [-1000,1000], 10层 Python ref golden | **10/10 PASS** |
| MODEL_B 真实模型 E2E | 灰度 112×112, 25层, INT8 cosine similarity | **cos=0.999953** |
| 转换器 C=1 修复 | 灰度图预处理 + 递归 calibration glob | ✓ |
| **DMA E2E 33-test 全回归** | **全部算子 INT8/INT16 + 全模型 + auto-next + IRQ** | **33/33 PASS** |

DMA E2E 33-test 全回归覆盖：

```
Tests  1-4 : 单层 Conv2D/DWConv INT8/INT16
Tests  5-6 : MobileNetV2-Tiny 10层 INT8/INT16
Tests  7-9 : 空间分块 32×32 INT8/INT8-tiled/INT16
Tests 10-13: Pooling (max 2×2, avg 2×2, global avg, max INT16)
Tests 14-16: Eltwise Add (basic, no_relu, INT16)
Tests 17-20: Resize (nearest 2×, bilinear 2×, nearest INT16, bilinear 3×3→5×5)
Tests 21-24: Deconv (2×2 s2, 3×3 s2, INT16, multichannel)
Tests 25-28: Concat (passthrough, rescale, INT16, 3-branch)
Tests 29-30: AllOps-Mini 18层全模型 INT8/INT16
Test  31   : AllOps-128 22次调用 128×128 全模型 (DMA tiling)
Test  32   : Auto-Next 3层自动推理
Test  33   : IRQ 中断驱动 E2E
```

验证环境：Verilator 5.036, 224M ns 仿真时间, 19m38s 壁钟时间 (~190K ns/s)。

### 14.10 Pre-CPU 集成测试加固
...
### 17.1 Xilinx 7-series 精确资源 (`synth_xilinx -family xc7`)

| 资源类型 | 数量 | 说明 |
|----------|------|------|
| **LUT (总计)** | **52,804** | LUT1~LUT6 合计 |
| — LUT6 | 18,144 | |
| — LUT5 | 12,704 | |
| — LUT3 | 10,803 | |
| — LUT2 | 6,202 | |
| — LUT1 | 2,876 | |
| — LUT4 | 2,075 | |
| **FF (触发器)** | **39,145** | FDCE 39,115 + FDPE 27 + FDRE 3 |
| **BRAM36E1** | **26** | 3 个 SRAM 映射 (ACT 32KB + WGT 64KB + PARAM 8KB = 104KB) |
| **DSP48E1** | **333** | PE array 乘法器 + PPU 流水线 |
| **CARRY4** | 8,022 | 进位链 |
| **MUXF7/F8** | 10,835 | 宽 MUX |
| **估计 Logic Cells** | **43,726** | |
| 综合时间 | 82s | 峰值内存 868 MB |

各模块资源分布：

| 模块 | Cells | 估计 LCs | DSP48 | 占比 |
|------|-------|----------|-------|------|
| npu_compute | 42,324 | 22,786 | 74 | 52% |
| npu_csr | 8,043 | — | 0 | 8% |
| npu_systolic (含 PE) | 6,084 | — | 256 | 6% |
| npu_dw_conv | 1,318 | — | 1 | 1% |
| npu_ppu | 904 | 569 | 2 | 1% |
| npu_dma | 344 | — | 0 | <1% |
| npu_ctrl | 264 | — | 0 | <1% |
| npu_sram (×3) | 32 | — | 0 | <1% (BRAM) |
| npu_top (interconnect) | 338 | — | 0 | <1% |

### 17.2 FPGA 器件适配

| 器件 | LUT | FF | BRAM36 | DSP48 | LUT 利用率 | 适配结论 |
|------|-----|-----|--------|-------|------------|---------|
| XC7A35T (Artix-7) | 20,800 | 41,600 | 50 | 90 | 254% | **不可用** — LUT/DSP 均不足 |
| **XC7A100T** (Artix-7) | 63,400 | 126,800 | 135 | 240 | **83%** | **基本可用** — DSP 333>240 需优化 |
| **XC7A200T** (Artix-7) | 134,600 | 269,200 | 365 | 740 | **39%** | **推荐 — 性价比首选** |
| XC7K70T (Kintex-7) | 41,000 | 82,000 | 135 | 240 | 129% | 偏紧 |
| **XC7K160T** (Kintex-7) | 101,400 | 202,800 | 325 | 600 | **52%** | **推荐 — 高主频选择** |
| XC7Z020 (Zynq) | 53,200 | 106,400 | 140 | 220 | 99% | 勉强 — DSP 不足 |
| **XC7Z045** (Zynq) | 218,600 | 437,200 | 545 | 900 | **24%** | **推荐 — 带 ARM SoC** |

> **DSP 优化方案**：当前 333 个 DSP48E1 主要来自 16×16 PE array (256) + PPU/Compute 乘法器 (77)。若目标器件 DSP 不足，可：
> 1. 降低 ARRAY_SIZE (8×8 → ~64 DSP，12×12 → ~144 DSP)
> 2. 使用 `synth_xilinx -nodsp` 将部分乘法器映射到 LUT
> 3. 优化 PPU 中的乘法器共享

### 17.3 Generic 综合参考 (`synth -top npu_top`)

| 资源 | 数量 | 说明 |
|------|------|------|
| 逻辑门 (AND/OR/XOR/NOT/MUX) | 846,550 | Generic gate-level |
| 触发器 (DFF) | 38,346 | |
| 存储块 ($mem_v2) | 3 | 104KB SRAM (未展开) |
| 综合时间 | 258s | 峰值内存 4.17 GB |
